...
impl Task {
    fn run_job_or_die(&self) {
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| self.job.run(&self.logger)))
            .unwrap_or_else(|e| {
                eprintln!("Panic in job runner: `{:?}`. Aborting.", e.type_id());
                std::process::abort();
            });
    }
}
...
mod tests {
    ...
    fn jobs_run() {
        ...
        runner.register(Box::new(job), Duration::from_millis(10));
        let stop = runner.stop.clone();

        std::thread::spawn(|| runner.start());
